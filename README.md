...
Um dos princípios centrais do projeto é a separação entre interpretação e tomada de decisão:
...
Representar uma linha de metrô como um grafo.

Entender na prática como funciona a BFS (Busca em Largura).

Implementar a DFS (Busca em Profundidade).

Comparar o comportamento dos dois algoritmos.

Aplicar lógica proposicional.

Criar uma pequena base de conhecimento com lógica de primeira
ordem.

Implementar um motor de inferência usando forward chaining.

Integrar regras lógicas com algoritmos de busca.

Usar um LLM para interpretar pedidos escritos em linguagem natural.

Criar um modo offline, sem depender de um LLM.

Gerar respostas em linguagem mais humana.

Exibir visualmente o resultado da busca.

Criar uma interface usando ipywidgets.

Validar o funcionamento com testes automatizados.
...
O funcionamento pode ser entendido de forma sequencial:
...
O usuário pode escrever a solicitação em linguagem natural, inclusive de forma informal, por exemplo:

to na se, quero ir para a Pinacoteca, tô de cadeira de rodas
...
O modo offline também funciona como uma espécie de alternativa de execução caso a
...
Essa visualização facilita a compreensão do comportamento dos algoritmos, porque permite enxergar o
...
A divisão em etapas organiza o notebook de forma didática:
...
aplicado a um problema bem definido: encontrar uma rota de metrô.
